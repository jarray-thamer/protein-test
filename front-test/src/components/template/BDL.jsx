    ...
    >
      <div>
        <div className="flex flex-col p-6 bg-white rounded-xl ">
          ...
          <div className="flex mt-8 sm:justify-start">
            <div className="w-full max-w-2xl space-y-2 sm:text-start">
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-1 sm:gap-1">
                {[
                  { label: "Nom de livreur:", value: data?.livreur?.name },

                  { label: "CIN de livreur:", value: data?.livreur?.cin },
                  {
                    label: "numéro d'immatriculation:",
                    value: data?.livreur?.carNumber,
                  },
                ].map((item, index) => (
                  <dl key={index} className="grid sm:grid-cols-2 gap-x-3">
                    <dt className="col-span-1 font-semibold text-gray-800 ">
                      {item.label}
                    </dt>
                    <dd className="col-span-1 text-gray-500 ">{item.value}</dd>
                  </dl>
                ))}
              </div>
            </div>
